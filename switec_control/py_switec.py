...
class SwitecX25(threading.Thread):
    ...
    def __init__(self):
        ...
        self.targetStep=None
        self.steps            # total steps available
        self.time0           // time when we entered this state

    self.microDelay;       // microsecs until next state
   unsigned short (*accelTable)[2]; // accel table can be modified.
self.maxVel;           // fastest vel allowed
self.vel;              // steps travelled under acceleration
self.dir;                      // direction -1,0,1
